...
class Channel:
        ...
        for n in range(0, nargin, 2):
            if varargin[n] == 'model':
                if varargin[n + 1] == 'umi-sc-los':
                    ...
                    self.model = {'alpha':2.8 , 'beta': 11.4, 'gamma': 2.3, 'sigma': 0.} #sigma-2.8, 4.1
                elif varargin[n + 1] == 'uma-nlos':
                    self.model = {'alpha': 3.3, 'beta': 25, 'gamma': 2.0, 'sigma': 0.}  #beta- 17.6, sigma-9.9 2.8
    ...
    def generate_paths(self, ch_randval):

        d = np.linalg.norm(self.tx - self.rx)
        self.pathloss[0] = self.ploss(d.reshape(1, 1))
        self.coeff[0] = np.sqrt(db2pow(-self.pathloss[0]))*ch_randval#np.exp(1j * 2 * np.pi * 0.6)#ch_randval#np.random.rand()
        self.tau[0] = d/self.c
        ...
        for n in range(0, self.nlos_path):
            ...
            d = np.linalg.norm(self.tx - self.sc[n]) + np.linalg.norm(self.rx - self.sc[n])
            self.pathloss[n + 1] = self.ploss(d.reshape(1, 1))
            self.coeff[n + 1] = np.sqrt(db2pow(-self.pathloss[n])) * np.exp(1j * 2 * np.pi * 0.3)#np.random.rand()

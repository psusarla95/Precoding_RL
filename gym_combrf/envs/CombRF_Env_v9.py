...
####################################
CombRF_Env - RF base station Environment with (TXBdir, RXbdir) beam pair and varying Beam Width
...
class CombRF_Env_v9(gym.Env):
    def __init__(self):
        ...
        self.rx_zcov = np.arange(1.5,22.5, 10)
        self.tx_beam = None

        self.aoa_min = 0
        self.aoa_max= 2*math.pi
        self.beamwidth_vec = np.array([np.pi / self.N_rx, 2 * np.pi / self.N_rx])#, , 2 * np.pi / self.N_rx, 2 * np.pi / self.N_rx,4*np.pi/self.N_rx
        self.BeamSet = Generate_Beams(self.N_rx, self.beamwidth_vec)  # Set of all beam directions
        self.ntx_actions =int(sum([self.N_tx/(2**i) for i in range(len(self.beamwidth_vec))]))
        self.nrx_actions = int(sum([self.N_rx/(2**i) for i in range(len(self.beamwidth_vec))]))
        self.action_space = spaces.Discrete(self.ntx_actions*self.nrx_actions)
        self.action = None
        self.goal_steps = 64  # cardinality of Beamset
        ...
        self.dqnaction_counter = [0 for i in range(self.ntx_actions*self.nrx_actions)]
    ...
    def step(self, action, ch_randval=None):
        ...
        tx_dir_ndx, rx_dir_ndx = int(action / self.ntx_actions), int(action % self.nrx_actions)
        ...
        self.obs = np.array(
            [np.concatenate((self.norm_tx_xloc, self.norm_tx_yloc, self.norm_tx_zloc), axis=0)])
    ...
    def reset(self, ch_randval):
        ...
        tx_loc_xndx, tx_loc_yndx, tx_loc_zndx =self.obs_space.sample()
        ...
        self.tx_num = self.get_txloc_ndx(self.tx_loc)
        self.channel.generate_paths(ch_randval, self.tx_num)
        self.dqnobs_counter[self.tx_num] += 1
        self.npaths = self.channel.nlos_path + 1
        self.h = self.channel.get_h() #channel coefficient

        #project TX in the transmitter direction
        #self.tx_beam = ula.steervec(self.N_tx, self.channel.az_aod[0], self.channel.el_aod[0])
        beam_ndx = self.action_space.sample()  # select a random receive direction
        tx_dir_ndx, rx_dir_ndx = int(beam_ndx / self.ntx_actions), int(beam_ndx % self.nrx_actions)
        self.tx_bdir = self.BeamSet[tx_dir_ndx]#self.channel.az_aod[0]#
        self.tx_beam = ula.var_steervec(self.N_tx, self.tx_bdir, 0)
    ...
    def test_reset(self, tx_loc, beam_ndx, sc, ch_randval):
        self.tx_loc = tx_loc
    ...
    def get_minmax_exhrate(self, ch_randval):
        ...
        for tbdir_ndx in range(self.ntx_actions):
            tx_bdir = self.BeamSet[tbdir_ndx]  # self.channel.az_aod[0]#
            tx_beam = ula.var_steervec(self.N_tx, tx_bdir, 0)
            eff_ch = np.array(self.h[:,:,0]).dot(tx_beam)

            for rbdir_ndx in range(self.nrx_actions):
                ...
                if rate > max_rate:
                    max_rate = rate
                    max_rxaction_ndx = rbdir_ndx
                    max_txaction_ndx = tbdir_ndx
                    max_rssi_val = rssi_val

                if rate < min_rate:
                    min_rate = rate
                    min_rxaction_ndx = rbdir_ndx
                    min_txaction_ndx = tbdir_ndx
                    min_rssi_val = rssi_val
        return min_rate, max_rate, (min_txaction_ndx, min_rxaction_ndx), (max_txaction_ndx, max_rxaction_ndx)

    def get_rssi_range(self):
        loc1 = np.array([[0,0,22.5]])
        sc = np.array([])#np.array([[-100,50,21.5], [-100,-50,21.5], [-50,100,21.5],[50,100,21.5]])#, [-50,100,11.5],[50,100,11.5]]np.array([[50,0,0], [-50,-100,0], [100,50,0],[50,-100,0]])#np.array([])
        ...
        max_rssi = 0
        for tx_dir_ndx in range(self.ntx_actions):
            tx_dir = self.BeamSet[tx_dir_ndx]
            tx_beam = ula.var_steervec(self.N_tx, tx_dir, 0)
            eff_ch = np.array(h[:, :, 0]).dot(tx_beam)
            for rx_dir_ndx in range(self.nrx_actions):
        ...
        sc = np.array([])#np.array([[-100,50,21.5], [-100,-50,21.5], [-50,100,21.5],[50,100,21.5]])#np.array([[-100,50,11.5], [-100,-50,11.5], [-50,100,11.5],[50,100,11.5]])#np.array([[50,0,0], [-50,-100,0], [100,50,0],[50,-100,0]])#np.array([])
        ch_model = 'uma-los'#'uma-nlos'

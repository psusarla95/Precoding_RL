...
####################################
CombRF_Env - RF base station Environment with varying Beam Width for LoS to NLoS changes
...
class CombRF_Env_v10(gym.Env):
    def __init__(self):
        ...
        self.rx_xcov = np.arange(-100*np.cos(53*np.pi/180), -1, self.rx_stepsize)#coverage along x axis
        self.rx_ycov = np.arange(-100*np.sin(53*np.pi/180), -1, self.rx_stepsize) #coverage along y axis
        ...
        self.reward_list = []
        self.temprwd_list = []
        self.dqnbestbeam_ndxlist = [0 for i in range(self.obs_space.nvec[3] * len(self.tx_locs))]
    ...
    def step(self, action, ch_randval=None):
        ...
        rwd, done, beam_flag, temp_rwd = self.get_reward_goal(self.rssi_val, beam_flag)
        ...
        return self.obs, rwd, done, temp_rwd, {}
    ...
    def get_rssi_range(self):
        ...
        sc = np.array([[-100,50,21.5], [-100,-50,21.5], [-50,100,21.5],[50,100,21.5]])#, [-50,100,11.5],[50,100,11.5]]np.array([[50,0,0], [-50,-100,0], [100,50,0],[50,-100,0]])#np.array([])
        ...
        sc = np.array([[-100,50,21.5], [-100,-50,21.5], [-50,100,21.5],[50,100,21.5]])#np.array([[-100,50,11.5], [-100,-50,11.5], [-50,100,11.5],[50,100,11.5]])#np.array([[50,0,0], [-50,-100,0], [100,50,0],[50,-100,0]])#np.array([])
        ch_model = 'uma-nlos'#'uma-nlos'
    ...
    def get_reward_goal(self, rssi_val, beam_flag):
        #transmission energy
        Es = db2lin(self.P_tx) #* (1e-3 / self.B)
        self.SNR = Es * np.abs(rssi_val)**2 / (self.N0*self.B)
        rate = np.log2(1 + self.SNR)  # in Gbit/s (self.B / self.nFFT) *
        rwd = -1.0#(float(np.around(rate-self.best_rate,decimals=2)))
        done = False

        #if ((self.rbdir_count+1) == self.goal_steps):#or ((rate == self.rate) and self.rbdir_count >1):  # ((rate >= self.rate) and (self.rbdir_count ==2)) or
        #    done = True
        ...
        self.rate = rate
        # print(rwd)
        return rwd, done, beam_flag, temp_rwd

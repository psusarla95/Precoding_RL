...
####################################
CombRF_Env_v6 - RF base station Environment for DQN fast fading environment
...
class CombRF_Env_v6(gym.Env):
    def __init__(self):
        ...
        self.sc_xyz = np.array([])#np.array([[-100,50,0], [-100,-50,0], [-50,100,0],[50,100,0]])#np.array([[50,0,0], [-50,-100,0], [100,50,0],[50,-100,0]])#np.array([[0,100,0], [10,50,0], [40,60,0], [70,80,0], [100,50,0], [80,85,0], [20,30,0], [10,40,0], [80,20,0]])#np.array([[0,100,0]])#np.array([[0,100,0],[250,0,0],[-200,-150,0]]) #reflection points for now
        ...
        self.action = None
        self.BeamSet = Generate_BeamDir(self.N_rx)  # Set of all beam directions
        ...
        self.dqnloc_minexhrates = [0 for i in range(self.action_space.n * len(self.tx_locs))]
    ...
    def step(self, action, ch_randval=None):
        ...
        tbdir_ndx = self.get_bdir_ndx(self.tx_bdir)
        self.dqnloc_maxexhselections[loc_ndx * self.action_space.n + tbdir_ndx] += 1
    ...
    def get_bdir_ndx(self, bdir):
        for ndx in range(len(self.BeamSet)):
            if self.BeamSet[ndx] == bdir:
                break
        return ndx
    ...
    def get_minmax_exhrate(self, ch_randvals):
        ...
        loc_ndx = self.get_txloc_ndx(self.tx_loc)
        tbdir_ndx = self.get_bdir_ndx(self.tx_bdir)
        eps_maxexhselections = self.dqnloc_maxexhselections[loc_ndx*self.action_space.n+tbdir_ndx]
        prev_maxexh_rate = self.dqnloc_maxexhrates[loc_ndx*self.action_space.n+tbdir_ndx]
        prev_minexh_rate = self.dqnloc_minexhrates[loc_ndx * self.action_space.n + tbdir_ndx]

        avg_max_rate = (prev_maxexh_rate * (eps_maxexhselections - 1) + avg_max_rate) / (eps_maxexhselections)
        avg_min_rate = (prev_minexh_rate * (eps_maxexhselections - 1) + avg_min_rate) / (eps_maxexhselections)

        self.dqnloc_minexhrates[loc_ndx*self.action_space.n+tbdir_ndx] = avg_min_rate
        self.dqnloc_maxexhrates[loc_ndx*self.action_space.n+tbdir_ndx] = avg_max_rate
    ...
    def get_rssi_range(self):
        ...
        sc = np.array([])#np.array([[-100,50,0], [-100,-50,0], [-50,100,0],[50,100,0]])#np.array([[50,0,0], [-50,-100,0], [100,50,0],[50,-100,0]])#np.array([])
        ch_model = 'uma-los'#'uma-nlos'

        channel = Channel(self.freq, loc1, self.rx_loc, sc, 'model', ch_model, 'nrx', self.N_rx,
                               'ntx', self.N_tx, 'nFFT', self.nFFT, 'df', self.df)

        channel.generate_paths(np.exp(1j * 2 * np.pi * 0.6)) #0.3 is some defined default value
        ...
        channel.generate_paths(np.exp(1j * 2 * np.pi * 0.6))
        h = channel.get_h()
    ...
    def get_reward_goal(self, rssi_val):
        ...
        if  (rate > self.rate):  #(rate > self.rate) and and (self.rbdir_count < 2)

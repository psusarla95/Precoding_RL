...
class CombRF_Env_v2(gym.Env):
    def __init__(self):
        ...
        self.dqnobs_counter = [0 for i in range(self.action_space.n * len(self.tx_locs))]
    ...
    def reset(self, ch_randval):
        ...
        self.tx_num = self.get_txloc_ndx(self.tx_loc)
        self.channel.generate_paths(ch_randval, self.tx_num)
        self.dqnobs_counter[self.tx_num*self.action_space.n+tx_dir_ndx] += 1
        self.npaths = self.channel.nlos_path + 1
        self.h = self.channel.get_h() #channel coefficient
    ...
    def retrain_reset(self, tx_loc, tbdir_ndx, rbdir_ndx, sc):
        self.tx_loc = tx_loc
        tx_dir_ndx = tbdir_ndx
